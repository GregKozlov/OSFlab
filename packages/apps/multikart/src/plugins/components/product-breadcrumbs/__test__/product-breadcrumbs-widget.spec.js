...
export const ProductBreadcrumbs = () => import('../index');

describe('ProductBreadcrumbs renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {ProductBreadcrumbs};
  beforeAll(async () => {
    /**
  ...
  it('ProductBreadcrumbs works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'ProductBreadcrumbs',
            regions: []
          }
        }
      }
