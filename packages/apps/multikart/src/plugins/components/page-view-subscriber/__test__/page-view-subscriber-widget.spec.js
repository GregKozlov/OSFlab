...
export const PageViewSubscriber = () => import('../index');

describe('PageViewSubscriber renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {PageViewSubscriber};
  beforeAll(async () => {
    /**
  ...
  it('PageViewSubscriber works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'PageViewSubscriber',
            regions: []
          }
        }
      }
